<template>
  <div class="grid-container-music-section">
    <div class="welcome">
      <div class="player-header"><h2>Good Morning</h2></div>
      <div>
        <div class="info-box">
          <div class="card-title">
            <div class="lx-s">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm">
              <a href=""><p>Rap UK</p></a>
            </div>
          </div>
        </div>
        <div class="info-box">
          <div class="card-title">
            <div class="lx-s">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm">
              <a href=""><p>Rap UK</p></a>
            </div>
          </div>
        </div>
      </div>
      <div>
        <div class="info-box">
          <div class="card-title">
            <div class="lx-s">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm">
              <a href=""><p>Rap UK</p></a>
            </div>
          </div>
        </div>
        <div class="info-box">
          <div class="card-title">
            <div class="lx-s">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm">
              <a href=""><p>Nigerian Top 100</p></a>
            </div>
          </div>
        </div>
      </div>
      <div>
        <div class="info-box">
          <div class="card-title">
            <div class="lx-s">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm">
              <a href=""><p>Rap UK</p></a>
            </div>
          </div>
        </div>
        <div class="info-box">
          <div class="card-title">
            <div class="lx-s">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm">
              <a href=""><p>Nicki Minaj</p></a>
            </div>
          </div>
        </div>
      </div>
      <div>
        <div class="info-box">
          <div class="card-title">
            <div class="lx-s">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm">
              <a href=""><p>Rap UK</p></a>
            </div>
          </div>
        </div>
        <div class="info-box">
          <div class="card-title">
            <div class="lx-s">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm">
              <a href=""><p>Queen</p></a>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="recent-played">
      <div class="card-header">
        <h2><a href="">Recent Played</a></h2>
      </div>
      <div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
      </div>
      <div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
      </div><div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
      </div><div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="made-for-you">
      <div class="card-header">
        <h2><a href="">Made for TommyWayne</a></h2>
      </div>
      <div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
      </div>
      <div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
      </div>
      <div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
      </div>
      <div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
        <div class="info-box-m">
          <div class="card-title-m">
            <div class="lx-s-m">
              <img src="../assets/FCMB.png" alt="card-img" />
            </div>
            <div class="lm-m">
              <a href="" class="card-big-title"><div>Rap UK</div></a>
              <div class="description">Description</div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "Music Section",
};
</script>

<style lang="scss" scoped>
.grid-container-music-section {
  display: block;
  width: 100%;
  height: 100%;
  grid-template-rows: 20% 30%;
  padding: 0px;
  overflow-y: auto;
  overflow-x: hidden;
  background-color:#121212;


  a {
    color: white;
    text-decoration: none;
    align-items: center;
    border-bottom: none;
    display: flex;
    flex: 1;
    justify-content: flex-start;
    overflow: hidden;
    align-items: center;

    p {
      align-items: center;
      padding: 0px 16px;
      overflow: hidden;
      text-overflow: ellipsis;
      white-space: nowrap;
      white-space: normal;
      word-break: break-word;
      // font-size: 16px;
      font-weight: 700;
      letter-spacing: normal;
      line-height: 24px;
      text-transform: none;
      margin-block-start: 1em;
      margin-block-end: 1em;
      margin-inline-start: 0px;
      margin-inline-end: 0px;
    }
  }

  .welcome {
    padding-bottom: 16px;
    width: 100%;
    // height: 30vh;
    background: transparent;
    grid-gap: 24px;
    display: grid;
    margin: 0;
    // max-width: 1955px;
    padding: 16px;
    padding-right: 20px;
    margin-bottom: 6px;
    min-height: 224px;
    position: relative;
    grid-template-rows: 10% 45% 45%;
    grid-template-columns: repeat(4, 1fr);

    .player-header {
      grid-column: 1/ -1;
    }

    .info-box {
      width: 90%;
      // @include padding-sm;
      padding: 0;
      background-color: hsla(0, 0%, 100%, 0.1);
      border-radius: 4px;
      height: 100px;
      position: relative;
      display: flex;
      flex-direction: row;
      justify-content: space-between;

      .card-title {
        display: flex;
        flex-direction: row;
        justify-content: space-between;

        .lx-s {
          min-height: 80px;
          min-width: 80px;
          background-color: hsla(0, 0%, 100%, 0.2);
          border-radius: 4px;
          // padding-bottom: 100%;
          position: relative;
          width: 40%;
          border-radius: 2px;
          left: 0;
          top: 0;

          .lm {
            align-items: center;
            display: flex;
            flex: 1;
            justify-content: space-between;
            padding: 16px;
            bottom: 0;
            cursor: pointer;
            left: 0;
            overflow: hidden;
            right: 10px;
            text-indent: 100%;
            top: 0;
            white-space: nowrap;

            a {
              color: #fff;
              text-decoration: none;
              color: #b3b3b3;
              cursor: default;
              text-decoration: none;
              align-items: center;
              border-bottom: none;
              display: flex;
              -ms-flex: 1;
              flex: 1;
              justify-content: flex-start;
              overflow: hidden;

              p {
                display: -webkit-box;
                overflow: hidden;
                text-overflow: ellipsis;
                white-space: nowrap;
                white-space: normal;
                word-break: break-word;
                font-size: 16px;
                font-weight: 700;
                letter-spacing: normal;
                line-height: 24px;
                text-transform: none;
                margin-block-start: 1em;
                margin-block-end: 1em;
                margin-inline-start: 0px;
                margin-inline-end: 0px;
              }
            }
          }
        }
      }
    }

    div {
      margin-bottom: 16px;
      min-width: 0;

      h2 {
        color: #fff;
        display: inline-block;
        max-width: 100%;
        overflow: hidden;
        text-overflow: ellipsis;
        white-space: nowrap;
        font-size: 32px;
        font-weight: 700;
        letter-spacing: -0.04em;
        line-height: 36px;
        text-transform: none;
      }
    }
  }
  .recent-played {
    grid-template-rows: 5% 45% 50%;
    grid-template-columns: repeat(4, 1fr);
    grid-gap: 24px;
    display: grid;
    margin: 0;
    max-width: 1955px;
    padding: 16px;

    .card-header {
      grid-column: 1/ -1;
    }
  }
  .made-for-you {
    grid-gap: 24px;
    display: grid;
    margin: 0;
    max-width: 1955px;
    padding: 16px;
    grid-template-rows: 5% 45% 55%;
    grid-template-columns: repeat(4, 1fr);

    .card-header {
      grid-column: 1/ -1;
    }
  }
}

.info-box-m {
  background: #181818;
  border-radius: 4px;
  flex: 1;
  isolation: isolate;
  padding: 16px;
  position: relative;
  width: 100%;
  margin: 16px 0px;

  .card-title-m {
    display: block;
    height: 100%;

    .lx-s-m {
      // min-height: 80px;
      // min-width: 80px;
      margin-bottom: 16px;
      position: relative;
      background-color: hsla(0, 0%, 100%, 0.2);
      border-radius: 4px;
      padding-bottom: 100%;
      position: relative;
      width: 100%;
      background-color: #333;
      padding-bottom: 100%;
      position: relative;
      width: 100%;

      img {
        display: block;
        border-radius: 2px;
        height: 100%;
        left: 0;
        position: absolute;
        top: 0;
        width: 100%;
      }

      .lm-m {
        min-height: 62px;

        a {
          color: #fff;
          text-decoration: none;
          position: relative;
          z-index: 1;
          display: inline-block;
          max-width: 100%;
          vertical-align: middle;
        }
      }
    }
  }
}

.description {
  color: rgb(179, 179, 179);
  margin-top: 4px;
  white-space: normal;
  overflow: hidden;
  text-overflow: ellipsis;
  font-size: 14px;
  font-weight: 400;
  letter-spacing: normal;
  line-height: 16px;
  text-transform: none;
}

.card-big-title {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
  font-size: 16px;
  font-weight: 700;
  letter-spacing: normal;
  line-height: 24px;
  text-transform: none;
}

.card-header {
  display: block;

  h2 {
    font-size: 24px;
    font-weight: 700;
    letter-spacing: -0.04em;
    line-height: 28px;
    text-transform: none;

    a {
      color: #fff;
      display: inline-block;
      max-width: 100%;
      overflow: hidden;
      text-overflow: ellipsis;
      white-space: nowrap;
    }
  }
}
</style>
